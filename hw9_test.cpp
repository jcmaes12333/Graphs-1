...
TEST(BasicListTest, RemoveRebalanceChecksRightCases) {
  ...
  // case 2: color flip
  RBTCollection<string,int> c4;
  for (int i = 0; i <= 17; ++i) {// swapped start and end i so it adds left every time
    string s = "";
    s += (char)(i + 97);
    c4.add(s, i+10);
  }
...
//Test 13
TEST(BasicListTest, RemoveRoot) {//Makes sure the collection can handle removing the root and stay valid
  RBTCollection<string,int> c1;
  c1.add("e",10);
  c1.add("d",20);
  c1.add("f",30);
